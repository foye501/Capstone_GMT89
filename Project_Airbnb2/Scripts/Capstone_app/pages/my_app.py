...
def run_model(my_capstone_model,
    ...
    try:
        loc_features = my_capstone_model.location_processor.process_new_data(overpy.Overpass(), address, no_guests , no_beds)
        loc_features[loc_features.columns] = np.where(np.isinf(loc_features[loc_features.columns]), -1, loc_features[loc_features.columns]) 
    except Exception as e:
        print(e)
        st.write('adddress not working, using default address')
        print('adddress not working, using default address')
        loc_features = my_capstone_model.location_processor.process_new_data(overpy.Overpass(),'1st Helms Ave, Culver City, California, United States', no_guests , no_beds)
